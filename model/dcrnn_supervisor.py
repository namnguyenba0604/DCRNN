...
class DCRNNSupervisor(object):
    ...
    def __init__(self, adj_mx, **kwargs):
        ...
        self._lr = tf.compat.v1.get_variable('learning_rate', shape=(), initializer=tf.compat.v1.constant_initializer(0.01),
                                   trainable=False)
        self._new_lr = tf.compat.v1.placeholder(tf.compat.v1.float32, shape=(), name='new_learning_rate')
        ...
        grads = tf.compat.v1.gradients(self._train_loss, tvars)
        max_grad_norm = kwargs['train'].get('max_grad_norm', 1.)
        grads, _ = tf.compat.v1.clip_by_global_norm(grads, max_grad_norm)
