...
class DCRNNModel(object):
    def __init__(self, is_training, batch_size, scaler, adj_mx, **model_kwargs):
        ...
        self._inputs = tf.compat.v1.placeholder(tf.compat.v1.float32, shape=(batch_size, seq_len, num_nodes, input_dim), name='inputs')
        # Labels: (batch_size, timesteps, num_sensor, input_dim), same format with input except the temporal dimension.
        self._labels = tf.compat.v1.placeholder(tf.compat.v1.float32, shape=(batch_size, horizon, num_nodes, input_dim), name='labels')

        # GO_SYMBOL = tf.compat.v1.zeros(shape=(batch_size, num_nodes * input_dim))
        GO_SYMBOL = tf.compat.v1.zeros(shape=(batch_size, num_nodes * output_dim))
        ...
        with tf.compat.v1.variable_scope('DCRNN_SEQ'):
            inputs = tf.compat.v1.unstack(tf.compat.v1.reshape(self._inputs, (batch_size, seq_len, num_nodes * input_dim)), axis=1)
            labels = tf.compat.v1.unstack(
                tf.compat.v1.reshape(self._labels[..., :output_dim], (batch_size, horizon, num_nodes * output_dim)), axis=1)
            ...
            def _loop_function(prev, i):
                if is_training:
                    ...
                    if use_curriculum_learning:
                        c = tf.compat.v1.random_uniform((), minval=0, maxval=1.)
                        threshold = self._compute_sampling_threshold(global_step, cl_decay_steps)
                        result = tf.compat.v1.cond(tf.compat.v1.less(c, threshold), lambda: labels[i], lambda: prev)
                    else:
                        result = labels[i]
                else:
            ...
            _, enc_state = tf.contrib.rnn.static_rnn(encoding_cells, inputs, dtype=tf.compat.v1.float32)
            outputs, final_state = legacy_seq2seq.rnn_decoder(labels, enc_state, decoding_cells,
                                                              loop_function=_loop_function)

        # Project the output to output_dim.
        outputs = tf.compat.v1.stack(outputs[:-1], axis=1)
        self._outputs = tf.compat.v1.reshape(outputs, (batch_size, horizon, num_nodes, output_dim), name='outputs')
    ...
    def _compute_sampling_threshold(global_step, k):
        ...
        :return:
        """
        return tf.compat.v1.cast(k / (k + tf.compat.v1.exp(global_step / k)), tf.compat.v1.float32)
